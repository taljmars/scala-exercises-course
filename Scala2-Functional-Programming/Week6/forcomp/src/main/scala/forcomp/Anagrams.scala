...
object Anagrams extends AnagramsInterface {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.groupBy(c=>c.toLower).map(x => (x._1, x._2.length)).toList.sorted
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    def helper(current: Sentence, leftOccurrence: Occurrences): List[Sentence]  = {
      if (leftOccurrence.isEmpty)
        ...
        val r = for {
          ...
          t = wordListForSentence.map(w => helper(current:::List(w), subtract(leftOccurrence, comb)))
        }
        yield t.flatten
        r.flatten
      }
    }
    helper(List(), lettersOcc)
  }
}
