...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    b.legalNeighbors.map{case (nBlock: Block, nMove: Move) => (nBlock, history:::List(nMove))}.to(LazyList)
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    case (block, listMoves) #:: tail =>
      val newNeighbors: LazyList[(Block, List[Move])] = newNeighborsOnly(neighborsWithHistory(block, listMoves), explored)
      (block, listMoves) #:: from(tail ++ newNeighbors, explored + block)
  }

  /**
   * The lazy list of all paths that begin at the starting block.
   */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = from(List((startBlock, List[Move]())).to(LazyList), Set())
  ...
  lazy val solution: List[Move] = pathsToGoal match {
    case LazyList() => List()
    case (_: Block, moves: List[Move]) #:: _ => moves
  }
}
